"""All rank-8 schemes for <2,2,2> over F2 as term SETS: SAT enumeration with lexicographically increasing
...
n=2; L=12; T=set(target_tensor(2,2,2)); k=8
st={"nv":0}; cl=[]
...
while s.solve():
    ...
    S=frozenset(sch); assert len(S)==k and verify(list(S),2,2,2); found.append(S)
...
print("rank-8 schemes for <2,2,2> over F2 (distinct term sets):",len(found),"in %.0fs"%(time.time()-t0),flush=True)
...
json.dump({"count":len(found),"classes":len(classes),"orbit_sizes":classes,"schemes":[[list(t) for t in sorted(S)] for S in found]},open("../results/all_222_rank8_F2.json","w"))
